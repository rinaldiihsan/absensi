...
const postPulangSiswa = (req, res) => {
  ...
  db.query(findSiswaSql, findSiswaValues, (err, result) => {
    if (err) {
      ...
      const insertPulangSiswaSql = 'INSERT INTO pulang_siswa (nis, nama_siswa,kelas, keterangan, latitude, longitude,tanggal) VALUES (?, ?, ?, ?, ?, ?, ?)';
      const insertPulangSiswaValues = [nis, nama_siswa, kelas, keterangan, latitude, longitude, tanggal];
...
const postPulangGuru = (req, res) => {
  ...
  db.query(findGuruSql, findGuruValues, (err, result) => {
    if (err) {
      ...
      const insertPulangGuruSql = 'INSERT INTO pulang_guru (nip, nama_guru, keterangan, latitude, longitude,tanggal) VALUES (?, ?, ?, ?, ?, ?)';
      const insertPulangGuruValues = [nip, nama_guru, keterangan, latitude, longitude, tanggal];
